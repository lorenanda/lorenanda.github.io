...
|------------------|------------------|
| Twitter          | `tweepy` & `vader`         |
| Slack            | `slackclient`    |
| [MongoDB]((https://account.mongodb.com/account/register))          | `pymongo`        |
| PostgreSQL       | `psycopg2-binary` & `sqlalchemy` |
| Docker-Compose   | - |

## 1\. Collecting tweets
To collect tweets, I used the [Twitter API](https://developer.twitter.com/en/docs/twitter-api) along with the [`tweepy` library](https://www.tweepy.org/).
...
Second, I wrote the [Python code for storing tweets in MongoDB](https://github.com/lorenanda/tweets-docker-pipeline/tree/main/docker-compose/tweet_collector) using the [`pymongo` library](https://pymongo.readthedocs.io/en/stable/).
...
To analyze the sentiment of the tweets, I used the [`VADER` library](https://pypi.org/project/vaderSentiment/) , which returns (among others) a compound sentiment score.
...
To load the tweets with their sentiment scores into a Postgres database, first you need––well, a Postgres database. I [installed Postgres](https://www.postgresql.org/download/), then created a database and a table for tweets right from the terminal:
...
The final touch of this project is *orchestration*. The individual Python scripts for each step work when you run them manually, but the goal is to run this pipeline from beginnning to end with only one command. This is where [**Docker Compose**](https://docs.docker.com/engine/reference/commandline/container/) comes in.
